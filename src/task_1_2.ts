...
let inputOne: InputType = prompt('Write something ');
let inputTwo: InputType = prompt('Write number or text ');
...
const parseData = (): InputsData | null => {
...
const storageData = () => {
  ...
  if (parsedData) {
    document.write(
      `<p> Field 1: ${parsedData.field1} <br> Field 2: ${parsedData.field2}</p>`
    );
  }
};
storageData();
