"use strict";
let amountMoney = parseFloat(prompt('Введіть суму .') || '1');
let inputCurrency = prompt('Введіть валюту "USD,EUR,UAH"') || 'USD';
const usdToUah = 41;
const eurToUah = 44;
const currencyValue = inputCurrency.toUpperCase();
const templateRender = (values) => {
...
if (!isNaN(amountMoney) && amountMoney > 0) {
    ...
    switch (currencyValue) {
        case 'USD':
            values.push({ uah: usdToUah * amountMoney }, { eur: (usdToUah * amountMoney) / eurToUah });
            templateRender(values);
            break;
        case 'EUR':
            values.push({ uah: eurToUah * amountMoney }, { usd: (eurToUah * amountMoney) / usdToUah });
            templateRender(values);
            break;
        case 'UAH':
            values.push({ eur: amountMoney / eurToUah }, { usd: amountMoney / usdToUah });
            templateRender(values);
            break;
        default:
